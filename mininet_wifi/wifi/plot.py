...
def get_wifi_node_color(node):
    color = 'b'
    from mininet_wifi.wifi.node import Station, Car, DockerStation, DockerCar
    if isinstance(node, Station) or isinstance(node, DockerStation):
        color = 'g'
    elif isinstance(node, Car) or isinstance(node, DockerCar):
        color = 'r'
    return color
...
class plot3d (object):
    ...
    def instantiateCircle(cls, node):
        ...
        color = get_wifi_node_color(node)
...
class plot2d (object):
    ...
    def instantiateNode(cls, node):
        ...
        color = color = get_wifi_node_color(node)
