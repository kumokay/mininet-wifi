...
from mininet.util import (quietRun, fixLimits, numCores, ensureRoot,
                          ...
                          waitListening)
from mininet.link import Link, Intf, TCLink #TCULink: containernet has no TCULink
from mininet.nodelib import NAT
from mininet.log import info, error, debug, output, warn

from mininet_wifi.wifi.node import AccessPoint, AP, Station, Car, \
    OVSKernelAP, physicalAP, DockerStation, DockerCar
from mininet_wifi.wifi.wmediumdConnector import WmediumdStarter, WmediumdServer, \
...
print('append path: {}'.format(str(os.getcwd()) + '/mininet_wifi/'))

sys.path.append(str(os.getcwd()) + '/mininet_wifi/')
...
class Mininet_wifi(Mininet):
    ...
    def ping(self, hosts=None, timeout=None):
        ...
        ploss = super(Mininet, self).ping(hosts)
        if self.sixLP:
            nodes = self.sixLP
    ...
    def iperf(self, hosts=None, l4Type='TCP', udpBw='10M', fmt=None,
        ...
        if isinstance(client, Station) or isinstance(server, Station) \
                or isinstance(client, DockerStation) or isinstance(server, DockerStation):
            if isinstance(client, Station) or isinstance(client, DockerStation):
                while conn1 is 0:
                    conn1 = int(client.cmd('iw dev %s link '
                                           '| grep -ic \'Connected\''
                                           % client.params['wlan'][0]))
            if isinstance(server, Station) or isinstance(client, DockerStation):
                while conn2 is 0:
    ...
    def configWirelessLinkStatus(self, src, dst, status):
        ...
        if isinstance(self.nameToNode[src], Station) or isinstance(self.nameToNode[src], DockerStation):
    ...
    def configLinkStatus(self, src, dst, status):
        ...
        if ((isinstance(self.nameToNode[src], Station) or isinstance(self.nameToNode[src], DockerStation))
                ...
                ((isinstance(self.nameToNode[dst], Station) or isinstance(self.nameToNode[dst], DockerStation))
                and isinstance(self.nameToNode[src], AP)):
            self.configWirelessLinkStatus(src, dst, status)
        else:
            super(Mininet, self).configLinkStatus(src, dst, status)
    ...
    def configureWifiNodes(self):
        ...
        for node in nodes:
            for wlan in range(0, len(node.params['wlan'])):
                if (isinstance(node, Car) or isinstance(node, DockerCar)) and wlan >= 1:
    ...
    def auto_association(self):
        ...
        for node in nodes:
            for wlan in range(0, len(node.params['wlan'])):
                if (isinstance(node, Car) and isinstance(node, DockerCar)) and wlan == 1:
                    node = node.params['carsta']
                    wlan = 0
        ap = []
